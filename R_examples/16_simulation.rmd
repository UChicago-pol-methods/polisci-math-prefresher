...
```{r}
sample(x = 1:10, replace = FALSE) ## no number appears more than once
```
...
```{r}
fair_coin <- sample(c("Head", "Tail"), size = 20, prob = c(.5, .5), replace = TRUE)
mean(fair_coin == "Head")
```
...
```{r}
unfair_coin <- sample(c("Head", "Tail"), size = 20, prob = c(2, 10), replace = TRUE)
mean(unfair_coin == "Head")
```
...
```{r}
mtcars[sample(1:ncol(mtcars), size=3),]
...
```{r}
from_runif <- runif(n = 100000)
from_rnorm <- qnorm(from_runif)
...
set.seed(60639)
mean(map_vec(1:1e5, function(x) radvantage() > 9))
```
...
```{r}
set.seed(60639)
mean(rcauchy(1e6)) # Average of 100,000 draws from a Cauchy(0, 1)
mean(rcauchy(1e6)) # Another average of 100,000 draws from a Cauchy(0, 1) -- wildly different resutl!
```
...
**Third**, the game decides whether to give the "banker hand" a third card. If the player did not receive a third card, the banker acts by the same rule as the player (draw if the banker's hand is 5 or less, stand if the banker's hand is 6 or 7). If the player did receive a third card, then the decision to draw depends on both the value of the banker's current hand and the value of the **third card drawn** by the player.
...
- If the bettor bet on "player" and the "banker" wins or if they bet "banker" and the "player" wins, they lose the amount wagered.
