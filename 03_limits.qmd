...
```{r, llnsim, fig.cap="As the number of coin tosses goes to infinity, the average probabiity of heads converges to 0.5", echo= FALSE, warning=FALSE}
#| label: fig-llnsim
set.seed(02138)
n <- 1e3
...
Graphing helps you make this point more clearly.  See the sequence of $n = 1, ...20$ for each of the three examples in @fig-seqabc.

```{r, seqabc, fig.cap = "Behavior of Some Sequences", echo = FALSE, warning=F}
#| label: fig-seqabc
seq <- 1:20
...
```{r, echo=FALSE, warning=FALSE, fig.cap="Functions which are not defined in some areas", warning = FALSE}
...
In @fig-contdiscont, we can see that the first two functions are continuous, and the next two are discontinuous. $f(x) = 1 + \frac{1}{x^2}$  is discontinuous at $x= 0$, and $f(x) = \text{floor}(x)$ is discontinuous at each whole number.


```{r, fig-contdiscont, warning=FALSE, echo=FALSE,fig.cap="Continuous and Discontinuous Functions", warning = FALSE}
