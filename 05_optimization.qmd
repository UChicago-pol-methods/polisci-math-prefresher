...
and for small $\mathbf h$, $R(\mathbf h)$ is negligible.  Rearranging, we get
$$f({\mathbf x}^*+\mathbf h)-f({\mathbf x}^*)\approx \frac{1}{2} \mathbf h^\top {\mathbf H(x^*)}
\mathbf h $$
